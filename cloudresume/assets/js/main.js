...
(function($) {
	...
	// Main.
		...
		async function updateCounter() {
			...
			counter.innerHTML = ` Welcome to the Future, Visitor #: ${data}`;
		}
		...
		*/
		...
		document.addEventListener('DOMContentLoaded', function() {
			const textToType = "WELCOME TO MY CLOUD SANCTUARY";
			...
			function typingEffect() {
				if (deleting) {
					if (i > 0) {
						typingContainer.innerHTML = textToType.substring(0, i - 1);
						i--;
						setTimeout(typingEffect, 50); // Speed of deleting
					} else {
						deleting = false;
						setTimeout(typingEffect, 200); // Pause before retyping
					}
				} else {
					if (i < textToType.length) {
						typingContainer.innerHTML += textToType.charAt(i);
						i++;
						setTimeout(typingEffect, 100); // Speed of typing
					} else {
						deleting = true;
